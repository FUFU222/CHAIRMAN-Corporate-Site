...
class CXmailform
{
    ...
    public function LoadDataVer1($strFileName)
    {
        ...
        foreach ($lines as $value) {
            ...
            if (isset($value[1])) {
                ...
                switch ($value[0]) {
                    case 'page_title':
                        $this->mBasePageTitle = $value[1];
                        break;    //ページタイトル
                    case 'form_name':
                        $this->mBasicFormName = $value[1];
                        break;    //フォーム名の取得
                    case 'move_url':
                        $this->mBasicMoveURL = $value[1];
                        break;    //遷移先URLの取得
                    case 'mail_address':
                        $this->mBasicMailaddress = $value[1];
                        break;    //送信先メールアドレスの取得
                    case 'mail_subject':
                        $this->mBasicMailSubject = $value[1];
                        break;    //送信メールの件名
                    case 'mail_limit_time':
                        $this->mBasicMailLimitTime = $value[1];
                        break;    //メールの連続送信を規制する時間
                }
                ...
                switch ($value[0]) {
                    case 'text':
                        $this->mFormViewBinder->AddItem(new CTextItem($value[2], $value[3], $value[4], $value[5], $value[1], $value[6]));
                        break;
                    case 'textarea':
                        $this->mFormViewBinder->AddItem(new CTextAreaItem($value[2], $value[3], $value[4], $value[5], $value[1], $value[6]));
                        break;
                    case 'select':
                        $this->mFormViewBinder->AddItem(new CSelectItem($value[2], $value[3], $value[4], $value[5], $value[1], $value[6]));
                        break;
                    case 'radio':
                        $this->mFormViewBinder->AddItem(new CRadioItem($value[2], $value[3], $value[4], $value[5], $value[1], $value[6]));
                        break;
                    case 'fullname':
                        $this->mFormViewBinder->AddItem(new CFullNameItem($value[2], $value[3], $value[4], $value[5], $value[1], $value[6]));
                        break;
                    case 'address':
                        $this->mFormViewBinder->AddItem(new CAddressItem($value[2], $value[3], $value[4], $value[5], $value[1], $value[6]));
                        break;
                    case 'mail':
                        $this->mFormViewBinder->AddItem(new CMailItem($value[2], $value[3], $value[4], $value[5], $value[1], $value[6]));
                        break;
                    case 'code':
                        $this->mFormViewBinder->AddItem(new CCodeItem($value[2], $value[3], $value[4], $value[5], $value[1], $value[6]));
                        break;
                    case 'sex':
                        $this->mFormViewBinder->AddItem(new CSexItem($value[2], $value[3], $value[4], $value[5], $value[1], $value[6]));
                        break;
                    case 'selarea':
                        $this->mFormViewBinder->AddItem(new CSelAreaItem($value[2], $value[3], $value[4], $value[5], $value[1], $value[6]));
                        break;
                }
    ...
    {
        if (isset($arrValues['page_title'])) {
            $this->mBasePageTitle = trim($arrValues['page_title']);         //ページタイトル
        } else {
            $this->mBasePageTitle = '';
        }
        if (isset($arrValues['form_name'])) {
            $this->mBasicFormName = trim($arrValues['form_name']);          //フォーム名の取得
        } else {
            $this->mBasicFormName = '';
        }
        if (isset($arrValues['move_url'])) {
            $this->mBasicMoveURL = trim($arrValues['move_url']);           //戻り先URL
        } else {
            $this->mBasicMoveURL = '';
        }
        if (isset($arrValues['mail_address'])) {
            $this->mBasicMailaddress = trim($arrValues['mail_address']);       //送信先メールアドレスの取得
        } else {
            $this->mBasicMailaddress = '';
        }
        if (isset($arrValues['mail_subject'])) {
            $this->mBasicMailSubject = trim($arrValues['mail_subject']);       //送信メールの件名
        } else {
            $this->mBasicMailSubject = '';
        }
        if (isset($arrValues['mail_limit_time'])) {
            $this->mBasicMailLimitTime = trim($arrValues['mail_limit_time']);    //メールの連続送信を規制する時間
        } else {
            $this->mBasicMailLimitTime = 3;
        }
        ...
        for ($i = 1; $i <= $num; $i++) {
            ...
            switch ($arrValues["type"][$name]) {
                case 'text':
                    $objData = new CTextItem($name, $arrValues["name"][$name], $arrValues["$name"], $need, $arrValues["check"][$name], $option);
                    break;
                case 'textarea':
                    $objData = new CTextareaItem($name, $arrValues["name"][$name], $arrValues["$name"], $need, $arrValues["check"][$name], $option);
                    break;
                case 'select':
                    $objData = new CSelectItem($name, $arrValues["name"][$name], $arrValues["$name"], $need, $arrValues["check"][$name], $option);
                    break;
                case 'checkbox':
                    $objData = new CCheckboxItem($name, $arrValues["name"][$name], $arrValues["$name"], $need, $arrValues["check"][$name], $option);
                    break;
                case 'radio':
                    $objData = new CRadioItem($name, $arrValues["name"][$name], $arrValues["$name"], $need, $arrValues["check"][$name], $option);
                    break;
                case 'fullname':
                    $objData = new CFullNameItem($name, $arrValues["name"][$name], $arrValues["$name"], $need, $arrValues["check"][$name], $option);
                    break;
                case 'address':
                    $objData = new CAddressItem($name, $arrValues["name"][$name], $arrValues["$name"], $need, $arrValues["check"][$name], $option);
                    break;
                case 'mail':
                    $objData = new CMailItem($name, $arrValues["name"][$name], $arrValues["$name"], $need, $arrValues["check"][$name], $option);
                    break;
                case 'code':
                    $objData = new CCodeItem($name, $arrValues["name"][$name], $arrValues["$name"], $need, $arrValues["check"][$name], $option);
                    break;
                case 'sex':
                    $objData = new CSexItem($name, $arrValues["name"][$name], $arrValues["$name"], $need, $arrValues["check"][$name], $option);
                    break;
                case 'selarea':
                    $objData = new CSelAreaItem($name, $arrValues["name"][$name], $arrValues["$name"], $need, $arrValues["check"][$name], $option);
                    break;
            }
            if (!is_null($objData)) {
...
//      $strTxt = $this->setPreviewTag( $strTxt );
    ...
    {
        if (isset($arrValues['auto_mail'])) {
            $this->mReturnMail->mIsUsed = trim($arrValues['auto_mail']);
        }
        if (isset($arrValues['rmail_subject'])) {
            $this->mReturnMail->mMailSubject = trim($arrValues['rmail_subject']);
        }
        if (isset($arrValues['rmail_body'])) {
            $this->mReturnMail->mMailBody = trim($arrValues['rmail_body']);
        }
        if (isset($arrValues['from_address'])) {
            $this->mReturnMail->mFromAddress = trim($arrValues['from_address']);
        }
    ...
    public function getOptionInputHtml($isChkError)
    {
        ...
        for ($i = 0; $i < 2; $i++) {
            if ($i == $this->mReturnMail->mIsUsed) {
                $strTxt = str_replace('{$used_auto_' . $i . '}', 'selected', $strTxt);
            } else {
                $strTxt = str_replace('{$used_auto_' . $i . '}', '', $strTxt);
            }
        }
    ...
    function TwoPostCheck($strIP, $strFileName)
    {
        ...
        for ($i = 0; $i < count($content_list); $i++) {
            ...
            if (trim($arr[0]) == "" || trim($arr[1]) == "" || trim($arr[2]) == "") {
                continue;
            }
    ...
    public static function _formSetFormTag()
    {
        ...
        if ($chkpos > 10) {
            $strURL = substr($strURL, 0, $chkpos);
        }
        $chkpos = strrpos($strURL, '/');
        if ($chkpos > 10) {
            $strURL = substr($strURL, 0, $chkpos + 1);
        }
    ...
    //------------------------------------------------------
    // private 関数
    private function _formBasicPageTitle()
    {
        return $this->mBasePageTitle;
    }   //ページタイトル
    private function _formBasicFormName()
    {
        return $this->mBasicFormName;
    }   //フォーム名の取得
    private function _formBasicMoveURL()
    {
        return $this->mBasicMoveURL;
    }   //遷移先URLの取得
    private function _formBasicMailaddress()
    {
        return $this->mBasicMailaddress;
    }   //送信先メールアドレスの取得
    private function _formBasicMailSubject()
    {
        return $this->mBasicMailSubject;
    }   //送信メールの件名
    private function _formBasicMailLimitTime()
    {
        return $this->mBasicMailLimitTime;
    } //メールの連続送信を規制する時間
